...
func (s *PostStorage) GetUserFeed(ctx context.Context, userID int64, fq PaginatedFeedQuery) ([]PostWithMetadata, error) {
	query := `
		SELECT p.id, p.user_id, p.content, p.tags, p.created_at, p.version, p.title, u.username, COUNT(c.id) AS comments_count
		FROM posts p
...
func (s *PostStorage) Delete(ctx context.Context, postID int64) error {
	query := `
